...
public class UnityIRC : EditorWindow
{
	...
	private static string ircLog = Application.dataPath + "/.../Logs/irc.log";
	private static string usrLog = Application.dataPath + "/../Logs/userList.log";
	private static string msgLog = Application.dataPath + "/../Logs/sendMsg.log";
	private static string runLock = Application.dataPath + "/../Logs/running.lock";
    ...
    private static void SetChatStyle(ref GUIStyle style, bool wordWrap, int padding, Color color)
    {
	...
	private void OnGUI()
	{
		try
		{
			if (bgColor == null)
			{
				...
				bgColor.SetPixel(0, 0, Color.black);// new Color(0.219607f, 0.219607f, 0.219607f));
				bgColor.Apply();
			}
